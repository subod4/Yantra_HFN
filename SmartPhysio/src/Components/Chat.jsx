  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:4000/api/suggest-exercises",
        {
          method: "POST",
          headers: {
      ...
      console.log("message: ", formData.details);
      ...
      const saveResponse = await fetch(
        "http://localhost:8000/recommendations/save",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    switch (step) {
      case 1:
        return (
          <input
            type="text"
            className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#6C9BCF] focus:border-transparent dark:bg-neutral-700 dark:text-white"
